 ...
 */
const getDistance = (coord1: Required<Point>, coord2: Required<Point>) => {
    return Math.sqrt(
...
const getMinDistance = (points: Point[], point: Required<Point>) => {
    const edgesDistances = points.reduce((acc, edge) => {
 ...
 */
const getMaxDistance = (grid: Grid, startAngle: number, endAngle: number, gridLength: number, innerRadius: number, outerRadius: number): {maxDistance: number; maxDistanceCoords: Point} => {
    let maxDistance = grid[0];
    let maxDistanceCoords = getGridPointPosition(0, 0, startAngle, endAngle, gridLength, innerRadius, outerRadius);
    ...
    for (let i = 0; i < grid.length; i++) {
        ...
        if (grid[i] > maxDistance) {
            maxDistance = grid[i];
            maxDistanceCoords = getGridPointPosition(x, y, startAngle, endAngle, gridLength, innerRadius, outerRadius);
        }
    }
 ...
 * @param outerRadius Donut outer radius
 */
const getGridPointPosition = (x: number, y: number, startAngle: number, endAngle: number, gridLength: number, innerRadius: number, outerRadius: number): Point => {
